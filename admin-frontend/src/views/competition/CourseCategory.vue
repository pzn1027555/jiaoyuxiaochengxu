...
<script>
import { getCategoryList, getTopLevelCategories, createCategory, updateCategory, deleteCategory } from '@/api/courseCategory'
...
export default {
  ...
  methods: {
    ...
    async loadParents(){
      try{ const r = await getTopLevelCategories(); const arr = (r && (r.data||r)) || []; this.parentOptions = Array.isArray(arr)? arr: [] }catch(e){ this.parentOptions = [] }
    },
    paginate(page){
    ...
    async handleDelete(row){ await this.$confirm(`确定删除分类“${row.categoryName}”吗？`,'提示',{type:'warning'}); const r = await deleteCategory(row.id); if(r && (r.code===200 || r.success===true)){ this.$message.success('删除成功'); this.loadData() } },
    async submit(){ this.$refs.formRef.validate(async valid=>{ if(!valid) return; const payload = { ...this.form }; if(payload.level !== 2){ payload.parentId = null } const r = payload.id? await updateCategory(payload.id, payload): await createCategory(payload); if(r && (r.code===200 || r.success===true)){ this.$message.success(payload.id? '更新成功':'创建成功'); this.dialogVisible=false; this.loadData() } }) },

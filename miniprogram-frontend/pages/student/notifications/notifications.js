...
Page({
  ...
  async markRead(e){ const id = e.currentTarget.dataset.id; await notificationApi.markRead({ id }); this.loadList() },
  async markAll(){ await notificationApi.markAll(); this.loadList() },
  openTarget(e){
    const type = e.currentTarget.dataset.type
    const relatedId = e.currentTarget.dataset.relatedid
    if (type === 'schedule') {
      if (relatedId) {
        wx.navigateTo({ url: `/pages/student/schedule-confirm/schedule-confirm?planId=${relatedId}&viewMode=student` })
      }
      return
    }
    if (type === 'money') {
